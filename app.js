...
function onGPSSuccess(pos) {
  ...
  // Weather: every 2 min
  if (Date.now() - state._lastWeatherUpdate > 120000) {
...
function playStation() {
  ...
  dom.radioAudio.play().then(() => {
    ...
    showToast("⚠ Error de reproducción (CORS)", "warning");
    stopRadio();
  });
}
...
function initEvents() {
  ...
  dom.btnTraffic.addEventListener("click", () => {
    ...
    if (state.trafficVisible) {
      state.map.setStyle("mapbox://styles/mapbox/navigation-day-v1");
      showToast("Tráfico activado", "");
    } else {
      state.map.setStyle(state.currentStyle);
    }
  });
