...
const dom = {
  ...
  routeStatusBar: $("route-status-bar"),
  routeStatusText: $("route-status-text"),
  btnShowNav: $("btn-show-nav"),
  ...
  btnEndRouteMenu: $("btn-end-route-menu"),
  btnMenuTraffic: $("btn-menu-traffic"),
  btnMenuRadio: $("btn-menu-radio"),
  ...
  trafficBanner: $("traffic-banner"),
  trafficBannerText: $("traffic-banner-text"),
...
function onRouteCalculated(e) {
  ...
  setSheetState("mid");
  setVisibleTrafficSummary(true);
  fitRouteView(route);
  updateRouteTraffic(route);
  showRouteStatusBar(distKm, etaMins);
  showNavPill();
  startNavPillLoop();
...
function onRouteClear() {
  ...
  setVisibleTrafficSummary(false);
  if (dom.trafficBanner) dom.trafficBanner.classList.add("hidden");
...
function initEvents() {
  ...
  if (dom.btnShowNav) {
    dom.btnShowNav.addEventListener("click", () => {
      showNavPill();
    });
  }
  ...
  if (dom.btnCloseMenu) {
    dom.btnCloseMenu.addEventListener("click", closeSideMenu);
  }
  if (dom.sideMenuOverlay) {
    dom.sideMenuOverlay.addEventListener("click", closeSideMenu);
  }
  if (dom.btnStartRoute) {
    dom.btnStartRoute.addEventListener("click", () => {
      closeSideMenu();
      const navInput = document.querySelector(".mapboxgl-ctrl-geocoder input");
      if (navInput) {
        navInput.focus();
        showToast("Introduce un destino para iniciar la ruta", "");
      } else {
        showToast("Busca un destino en el mapa", "warning");
      }
    });
  }
  if (dom.btnEndRouteMenu) {
    dom.btnEndRouteMenu.addEventListener("click", () => {
      state.directions.removeRoutes();
      onRouteClear();
      closeSideMenu();
      showToast("Ruta finalizada", "");
  ...
  if (dom.btnMenuTraffic) {
    dom.btnMenuTraffic.addEventListener("click", () => {
      closeSideMenu();
      dom.btnTraffic.click();
    });
  }
  if (dom.btnMenuRadio) {
    dom.btnMenuRadio.addEventListener("click", () => {
      closeSideMenu();
