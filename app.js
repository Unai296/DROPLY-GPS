...
const dom = {
  ...
  btnAddStation: $("btn-add-station"),
  weatherIcon: $("weather-icon"),
  weatherTemp: $("weather-temp"),
  weatherDesc: $("weather-desc"),
  weatherHum: $("weather-hum"),
  ...
  routeTrackLabel: $("route-track-label"),
  routeSegSafe: $("route-seg-safe"),
  routeSegWarning: $("route-seg-warning"),
  routeSegDanger: $("route-seg-danger"),
  routeAlerts: $("route-alerts"),
  routeAlertType: $("route-alert-type"),
  routeAlertText: $("route-alert-text"),
...
window.addEventListener("error", event => {
  ...
  showToast("Error interno, recarga la página o usa un servidor local", "error", 8000);
  dom.splash?.classList?.add("hidden");
  dom.app?.classList?.remove("hidden");
});
window.addEventListener("unhandledrejection", event => {
  console.error("Unhandled rejection:", event.reason);
  showToast("Error interno de la app", "error", 8000);
  dom.splash?.classList?.add("hidden");
  dom.app?.classList?.remove("hidden");
});
...
async function runSplash() {
  ...
  if (location.protocol === "file:") {
    state.skipMap = true;
    showToast("Abre la app con un servidor local para que mapas y GPS funcionen mejor", "warning", 7000);
  }
  ...
  try {
    ...
    console.error(err);
    showToast("Error iniciando Droply GPS", "error", 8000);
  }
}
...
function applyCurrentMapStyle() {
  const style = state.trafficVisible ? getTrafficStyle() : state.currentStyle;
  if (state.map?.setStyle) {
    state.map.setStyle(style);
  }
}

function initTour() {
  if (localStorage.getItem("droply_tour_shown")) return;
  dom.tourOverlay.classList.remove("hidden");
}

function closeTour() {
  dom.tourOverlay.classList.add("hidden");
  localStorage.setItem("droply_tour_shown", "1");
}

function generateRouteAlerts(route) {
  const distanceMeters = route.distance;
  const estimatedNormalSeconds = (distanceMeters / 1000) / 50 * 3600;
  const ratio = route.duration / Math.max(1, estimatedNormalSeconds);

  let status = {
    label: "Ruta despejada",
    detail: "No hay incidentes ni retrasos importantes.",
    level: "safe",
  };

  const steps = route.legs?.[0]?.steps || [];
  const congestionValues = steps
    .map(step => step.congestion)
    .filter(Boolean);
  const hasHeavyCongestion = congestionValues.some(value => ["heavy", "severe", "very_heavy"].includes(value));

  if (ratio > 1.4 || hasHeavyCongestion) {
    status = {
      label: "Retraso en ruta",
      detail: "Tráfico denso o retenciones en tu ruta. Maneja con cuidado.",
      level: "warning",
    };
  }

  if (ratio > 1.8 || congestionValues.some(value => value === "severe")) {
    status = {
      label: "Accidente/obstrucción",
      detail: "Atención: hay tráfico muy lento o un incidente en la ruta.",
      level: "danger",
    };
    if (state.voiceEnabled) speak("Atención, tráfico muy lento o incidente en la ruta. Procede con precaución.");
  }
...
function renderRouteStatus() {
  const status = state.routeStatus || { label: "Ruta despejada", detail: "No hay incidentes.", level: "safe" };
  dom.routeTrackLabel.textContent = status.label;
  dom.routeAlertType.textContent = status.level === "danger" ? "Accidente" : status.level === "warning" ? "Aviso" : "Libre";
  dom.routeAlertText.textContent = status.detail;
  dom.routeAlerts.classList.toggle("hidden", status.level === "safe");
  ...
  const [safeWidth, warnWidth, dangerWidth] = widths[status.level] || widths.safe;
  dom.routeSegSafe.style.width = `${safeWidth}%`;
  dom.routeSegWarning.style.width = `${warnWidth}%`;
  dom.routeSegDanger.style.width = `${dangerWidth}%`;
}
...
// ─────────────────────────────────────────────
function initMap() {
  if (state.skipMap || !window.mapboxgl || !window.MapboxDirections) {
    if (state.skipMap) {
      showToast("Modo local activado: mapas desactivados en file://", "info", 7000);
    } else {
      showToast("Mapa no disponible: comprueba conexión o abre en servidor HTTPS", "warning", 7000);
    }
    state.map = createDummyMap();
  ...
  state.map.on("error", event => {
    showToast("Error cargando el mapa", "error", 5000);
...
function onMapLoaded() {
  if (!state.map?.getLayer || !state.map?.getSource) return;
  // Add 3D buildings only once and only when source exists
  if (state.map.getLayer("3d-buildings") || !state.map.getSource("composite")) return;
...
function onGPSSuccess(pos) {
  ...
  if (state.isFollowing && state.autoCenter && state.map?.easeTo) {
    state.map.easeTo({ center: [lng, lat], duration: 800 });
  }

  if (state.directions?.setOrigin) {
  ...
  // Weather: every 2 min
  if (Date.now() - state._lastWeatherUpdate > 300000) {
    fetchWeather(lat, lng);
    state._lastWeatherUpdate = Date.now();
  }
}
...
function updateHUD() {
  ...
  dom.speedBadge.classList.toggle("fast", state.speed > 120);
  dom.speedBadge.classList.toggle("medium", state.speed > 80 && state.speed <= 120);

  // Stats
  dom.altVal.textContent = state.altitude || "—";
  dom.accVal.textContent = state.accuracy || "—";

  if (state.userPos) {
    dom.coordsDisplay.textContent =
      `${state.userPos[1].toFixed(5)}°N  ${state.userPos[0].toFixed(5)}°E`;
  }

  // Speedometer visibility
  dom.speedBadge.style.display = state.showSpeedometer ? "flex" : "none";
}
...
function onRouteClear() {
  ...
  dom.routeAlerts.classList.add("hidden");
  dom.routeTrackLabel.textContent = "Sin ruta";
  const badge = $("route-active-badge");
...
function initEvents() {
  ...
  // Radio
  dom.btnPlay.addEventListener("click", toggleRadio);
  dom.btnNext.addEventListener("click", () => {
    state.radioIndex = (state.radioIndex + 1) % state.stations.length;
    renderStations();
    if (state.radioPlaying) playStation();
  });
  dom.btnPrev.addEventListener("click", () => {
    state.radioIndex = (state.radioIndex - 1 + state.stations.length) % state.stations.length;
    renderStations();
    if (state.radioPlaying) playStation();
  });
  dom.btnAddStation.addEventListener("click", () => {
    const url = dom.customUrl.value.trim();
    if (!url.startsWith("http")) { showToast("URL inválida", "warning"); return; }
    const name = prompt("Nombre de la emisora:") || "Emisora personalizada";
    state.stations.push({ name, url, genre: "Custom", icon: "📡" });
    renderStations();
    dom.customUrl.value = "";
    showToast(`✓ "${name}" añadida`, "success");
  });
  ...
  dom.btnTraffic.addEventListener("click", () => {
    ...
    applyCurrentMapStyle();
    showToast(state.trafficVisible ? "Tráfico real activado" : "Tráfico real desactivado", "");
  });
  ...
  dom.btnClearRoute.addEventListener("click", () => {
    if (state.directions?.removeRoutes) state.directions.removeRoutes();
    onRouteClear();
  ...
  dom.btnTheme.addEventListener("click", () => {
    ...
    if (state.trafficVisible) applyCurrentMapStyle();
    showToast(state.theme === "dark" ? "Tema oscuro" : "Tema claro", "");
  });
  ...
  dom.tourClose.addEventListener("click", closeTour);

  // Segment controls (units)
  document.querySelectorAll(".segment[data-setting]").forEach(btn => {
    btn.addEventListener("click", () => {
      const setting = btn.dataset.setting;
      document.querySelectorAll(`.segment[data-setting="${setting}"]`)
  ...
  dom.btnClearHistory.addEventListener("click", () => {
    state.routeHistory = [];
    localStorage.removeItem("droply_history");
    renderHistory();
...
try {
  ...
  console.error("Error al iniciar runSplash:", err);
  showToast("Error iniciando la aplicación", "error", 8000);
  showFallbackUI();
}
